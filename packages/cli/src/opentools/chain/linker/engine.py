...
class AsyncLinkerEngine:
    ...
    Parallel implementation to ``LinkerEngine``. Sync callers continue
    using ``LinkerEngine`` until their own task in Phase 2+ converts
    them (drain worker, AsyncChainBatchContext, async CLI rebuild,
    subscriptions). The sync class is deleted in Task 30.

    Behaviorally the two classes produce equivalent output on the same
    DB — the rule application loop body is copied verbatim from
    ``LinkerEngine.link_finding``. Only the data-access layer changes:
    raw SQL calls via ``store.execute_*`` are replaced with protocol
    methods (``fetch_findings_by_ids``, ``entities_for_finding``,
    ...
    # Mirrors LinkerEngine._MIN_SCOPE_FOR_IDF — below this many findings
    # every shared entity has near-zero IDF so we disable IDF weighting.
    _MIN_SCOPE_FOR_IDF: int = 5
    ...
    async def link_finding(
        ...
        *,
        ...
        Port of ``LinkerEngine.link_finding``. Rule application body is
        identical — only store access differs. Uses protocol lifecycle
        methods (``start_linker_run`` / ``finish_linker_run`` /
        ``set_run_status``) instead of the sync class's direct INSERT.
        """
        start = time.monotonic()
        ...
        # (replaces sync version's per-partner _load_finding loop).
        ...
        # 4. Apply rules per partner — BODY COPIED VERBATIM FROM SYNC
        # LinkerEngine.link_finding (only partner lookup differs).
        relations_to_upsert: list[FindingRelation] = []
        relations_updated = 0
        relations_skipped_sticky = 0
        rule_stats: dict[str, dict] = {}
