"""Anthropic API LLM extraction provider.

Uses ``anthropic.AsyncAnthropic`` for the production path. The API key is
read from the ``ANTHROPIC_API_KEY`` environment variable at call time (not
at import time) so tests that don't set the variable do not fail on import.

Tests inject ``call_fn`` (async str->str) to avoid real API calls.
"""
...
class AnthropicAPIProvider:
    """Extraction provider backed by the Anthropic Messages API.

    Production path uses ``anthropic.AsyncAnthropic`` with
    ``ANTHROPIC_API_KEY`` from the environment.
    Unit tests inject ``call_fn`` to skip real calls.
    """
