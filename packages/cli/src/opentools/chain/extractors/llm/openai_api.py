"""OpenAI API LLM extraction provider.

Uses ``openai.AsyncOpenAI`` for the production path. The API key is read
from the ``OPENAI_API_KEY`` environment variable at call time (not at
import time) so tests that don't set the variable do not fail on import.
...
class OpenAIAPIProvider:
    ...
    Production path uses ``openai.AsyncOpenAI`` with ``OPENAI_API_KEY``
