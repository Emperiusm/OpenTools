...
class DashboardState:
    ...
    def refresh_selected(self, needs: set[str] | None = None) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            if "findings" in needs:
                cur_count = self.store._conn.execute(
                    "SELECT COUNT(*) FROM findings WHERE engagement_id = ? AND deleted_at IS NULL",
                    (self.selected_id,),
                ).fetchone()[0]
                if cur_count != self._last_finding_count:
                    self.findings = self.store.get_findings(self.selected_id)
                    self._last_finding_count = cur_count

            if "timeline" in needs:
                cur_tl = self.store._conn.execute(
                    "SELECT COUNT(*) FROM timeline_events WHERE engagement_id = ?",
                    (self.selected_id,),
                ).fetchone()[0]
                if cur_tl != self._last_timeline_count:
                    self.timeline = self.store.get_timeline(self.selected_id)
                    self._last_timeline_count = cur_tl

            if "iocs" in needs:
                cur_ioc = self.store._conn.execute(
                    "SELECT COUNT(*) FROM iocs WHERE engagement_id = ?",
                    (self.selected_id,),
                ).fetchone()[0]
                if cur_ioc != self._last_ioc_count:
                    self.iocs = self.store.get_iocs(self.selected_id)
                    self._last_ioc_count = cur_ioc
    ...
    def delete_engagement(self, engagement_id: str) -> None:
        ...
        with self._lock:
            ...
            self._last_timeline_count = -1
            self._last_ioc_count = -1
    ...
    def add_finding(self, engagement_id: str, tool: str, title: str,
        ...
        with self._lock:
            ...
            self._last_finding_count = -1  # invalidate cache
    ...
    def add_ioc(self, engagement_id: str, ioc_type: str, value: str,
        ...
        with self._lock:
            ...
            self._last_ioc_count = -1  # invalidate cache
    ...
    async def start_container(self, name: str) -> None:
        ...
        Runs the blocking docker-compose call in a thread to avoid
        poisoning the Textual event loop.
        """
        if self.container_mgr is not None:
            import asyncio
            await asyncio.to_thread(self.container_mgr.start, [name])

    async def stop_container(self, name: str) -> None:
        """Stop a named container via ContainerManager."""
        if self.container_mgr is not None:
            import asyncio
            await asyncio.to_thread(self.container_mgr.stop, [name])

    async def restart_container(self, name: str) -> None:
        """Restart a named container via ContainerManager."""
        if self.container_mgr is not None:
            import asyncio
            await asyncio.to_thread(self.container_mgr.restart, [name])
