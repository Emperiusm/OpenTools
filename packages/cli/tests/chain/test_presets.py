...
def _seed(engagement_store, chain_store, findings_data):
...
def test_lateral_movement_runs_without_error(engagement_store_and_chain):
    ...
    _seed(engagement_store, chain_store, findings)
...
def test_priv_esc_chains_runs(engagement_store_and_chain):
    engagement_store, chain_store, _ = engagement_store_and_chain
    findings = [
    ...
    cache = GraphCache(store=chain_store, maxsize=4)
    results = priv_esc_chains(
        "eng_test", cache=cache, store=chain_store, config=ChainConfig(),
    )
    assert isinstance(results, list)


def test_external_to_internal_runs(engagement_store_and_chain):
    engagement_store, chain_store, _ = engagement_store_and_chain
    findings = [
    ...
    cache = GraphCache(store=chain_store, maxsize=4)
    results = external_to_internal(
        "eng_test", cache=cache, store=chain_store, config=ChainConfig(),
    )
    assert isinstance(results, list)


def test_crown_jewel_runs(engagement_store_and_chain):
    engagement_store, chain_store, _ = engagement_store_and_chain
...
def test_mitre_coverage_basic(engagement_store_and_chain):
    engagement_store, chain_store, _ = engagement_store_and_chain
    f = _finding("mc_a", description="uses T1566 for initial access and T1059 for execution")
    _seed(engagement_store, chain_store, [f])

    result = mitre_coverage("eng_test", store=chain_store)
...
def test_mitre_coverage_empty_engagement(engagement_store_and_chain):
    engagement_store, chain_store, _ = engagement_store_and_chain
